        ...
        ScrollView {
            VStack {
                ...
                Text ("You must obtain OSHC for the proposed duration of your student visa and renew your OSHC if you extend the length of your visa.").font(.system(size: 16, weight: .light, design: .serif))
                    .italic()
                Text("Can I transfer to a different OSHC provider?").font(.system(size: 22, weight: .bold, design: .rounded)).padding()
                
                Image("HealthImage13").resizable().aspectRatio(contentMode: .fit)
                
                Text ("You can transfer to a different OSHC provider at any time, but some providers charge a ‘refund processing fee’ if you transfer to a new policy before your old one ends.").font(.system(size: 16, weight: .light, design: .serif)).italic().padding()
